from sqlalchemy import Boolean, Column, String, DateTime, func, JSON, Enum, CheckConstraint
...
class Document(Base):
    ...
    entity_type = Column(Enum(EntityTypeEnum, native_enum=False), nullable=False)
    ...
    is_profile_photo = Column(Boolean, nullable=False, default=False)
    metadata_ = Column("metadata", JSON, default={})
    file_url = Column(String, nullable=False)
    cloudinary_public_id = Column(String(255), nullable=False)
    status = Column(
        Enum(VerificationStatusEnum, native_enum=False),
